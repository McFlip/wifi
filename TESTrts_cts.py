...
with open(trafficfile, 'r') as tf:
    ...
    while(1):
      ...
      if(networkState[nodeWhoGetsTurn][0] == 0):	#if done waiting for packet
        ...
        networkState[nodeWhoGetsTurn][1] = rtsTime + sifsTime + ctsTime
        networkState[nodeWhoGetsTurn][2] = 0
        isBusy = 1
        sending += 1
        ...
        if(collision): #do binary backoff and reset to waiting for DIFS
          ...
          if(sending == 0):
            collision = 0
            isBusy = 0
            timeMediaBusy += rtsTime + sifsTime
        else: #no collision
          ...
          networkState[nodeWhoGetsTurn][1] = sifsTime + ceildiv(waiting_qwee[nodeWhoGetsTurn][0][3],dataRate) + sifsTime + ackTime
      elif(networkState[nodeWhoGetsTurn][0] == 4):	#if done waiting for ACK\
        timeMediaBusy += rtsTime + ctsTime + ceildiv(waiting_qwee[nodeWhoGetsTurn][0][3],dataRate) + ackTime
